...
class _HomeScreenState extends State<HomeScreen> {
                ...
                background: Container(
                  ...
                  child: !_isScrolled
                    ...
                    padding: EdgeInsets.only(
                      ...
                      right: size.width * 0.05,
                      left: size.width * 0.05,
                      bottom: size.height * 0.01, // Add some bottom padding
                    ),
    ...
    return GestureDetector(
      onTap: () async {
        final result = await showModalBottomSheet(
          context: context,
          isScrollControlled: true,
          backgroundColor: theme.cardColor, // Use theme.cardColor here
          shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.vertical(
              top: Radius.circular(20),
            ),
      ...
      child: Container(
        ...
        padding: EdgeInsets.all(size.width * 0.04),
        decoration: BoxDecoration(
          // Explicitly set white for light mode, use theme.cardColor for dark
          color: isDarkMode ? theme.cardColor : Colors.white,
          borderRadius: BorderRadius.circular(12),
          boxShadow: [
            BoxShadow(
              // Use theme.shadowColor for consistency
              color: theme.shadowColor.withOpacity(isDarkMode ? 0.3 : 0.2),
              blurRadius: 5,
              spreadRadius: 1,
              offset: const Offset(0, 2),
            ),
        ...
        child: Row(
          ...
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            // Wrap the text in Flexible to prevent overflow if text is too long
            Flexible(
              child: ShaderMask(
                shaderCallback: (bounds) => buttonGradient.createShader(bounds),
                blendMode: BlendMode.srcIn,
                child: Text(
                  ...
                  style: TextStyle(
                    fontSize: 25,
                    fontWeight: FontWeight.bold,
                    color: Colors.white, // Assuming gradient needs a base color
                  ),
                  overflow: TextOverflow.ellipsis, // Add ellipsis for overflow
                  maxLines: 2, // Allow up to two lines if needed
                ),
              ),
            ),
            SizedBox(width: size.width * 0.02), // Add some spacing
            ShaderMask(
              shaderCallback: (bounds) => buttonGradient.createShader(bounds),
              blendMode: BlendMode.srcIn,
              child: Image.asset(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            ...
            textAlign: TextAlign.center,
            style:
            TextStyle(fontSize: 16, color: theme.textTheme.bodySmall?.color?.withOpacity(0.7)),
          ),
        ),
        SizedBox(height: size.height * 0.02),
        ElevatedButton(
  ...
  Widget _buildSkeletonOrderList(Size size) {
    final isDarkMode = AdaptiveTheme.of(context).mode == AdaptiveThemeMode.dark;
    return Column(
      children: List.generate(3, (index) => _buildSkeletonOrderItem(size, isDarkMode)),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Flexible(child: buildSkeletonLine(size.width * 0.4, 16, borderRadius: BorderRadius.circular(8))),
              SizedBox(width: 8),
              buildSkeletonLine(size.width * 0.2, 16, borderRadius: BorderRadius.circular(8)),
            ],
          ),
  ...
  Widget _buildSkeletonVehicleList(Size size) {
    final isDarkMode = AdaptiveTheme.of(context).mode == AdaptiveThemeMode.dark;
    return Column(
      children: List.generate(3, (index) => _buildSkeletonVehicleItem(size, isDarkMode)),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(width: size.width * 0.04),
          buildSkeletonLine(size.width * 0.15, size.height * 0.06, borderRadius: BorderRadius.circular(8)),
        ],
      ),
    );
  }
